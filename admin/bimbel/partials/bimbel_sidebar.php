...
<div id="sidebar" class="fixed inset-y-0 left-0 z-50 w-64 bg-gray-800 transform -translate-x-full transition-transform duration-300 ease-in-out lg:translate-x-0 lg:inset-0">
    ...
    <nav class="mt-5 px-2 space-y-1">
        <?php foreach ($menu_items as $item): ?>
            ...
            <?php if (($item['type'] ?? '') === 'button'): ?>
                <button id="<?= $item['id'] ?>" class="<?= $item['class'] ?>" aria-label="<?= $item['title'] ?>">
                    <i class="<?= $item['icon'] ?>"></i>
                </button>
            <?php else: ?>
                <a href="<?= $item['url'] ?>"
                   class="<?= $item['active'] ? 'bg-green-600 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white' ?>
    ...
    <div class="absolute bottom-0 w-full p-4 bg-gray-900">
        <div class="flex items-center">
            ...
            <div class="ml-3 sidebar-text">
                <p class="text-sm text-white truncate"><?= htmlspecialchars($current_user['full_name']); ?></p>
                <p class="text-xs text-gray-300"><?= ucfirst(str_replace('_', ' ', $current_user['role'])); ?></p>
            </div>
        </div>
    </div>
</div>
